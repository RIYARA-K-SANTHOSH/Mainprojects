    ...
    if request.method == 'POST':
        ...
        if cropped_image_data:
            try:
                ...
                detected_gender = result['gender'].lower()
                ...
                normalized_detected_gender = gender_mapping.get(detected_gender, 'other')
                
                if normalized_detected_gender != user_gender and user_gender != 'other':
                    messages.error(
                        request, 
                        f"Profile picture cannot be uploaded. Detected gender ({detected_gender}) "
                        f"does not match your registered gender ({user_gender})."
                    )
                    return redirect('uploadphoto', connect_id=connect_id)
                
                ProfilePicture.objects.filter(user=user, is_active=True).update(is_active=False)
                new_picture = ProfilePicture(user=user, image=image_file)
                new_picture.save()
                
                messages.success(request, f"Profile picture updated! Detected gender: {result['gender']}")
            ...
            try:
                ...
                ProfilePicture.objects.filter(user=user, is_active=True).update(is_active=False)
                new_picture = ProfilePicture(user=user, image=request.FILES['profile_picture'])
                new_picture.save()
...
def payment_history(request, connect_id):
    # Get user profile
    user_profile = get_object_or_404(UserProfile, connect_id=connect_id)
    
    # Get all payments for this user, ordered by most recent first
    payments = Payment.objects.filter(user_profile=user_profile).order_by('-paid_at')
    
    # Get current active plan (if any)
    active_payment = payments.filter(status='Completed').first()
